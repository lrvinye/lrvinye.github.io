---
title: 通过 Helm 部署 Postgres 集群
date: 2021-04-01 20:50:45
tags: [Helm, K8S, Postgres, Bitnami, 安装, 配置]
category: 运维
---

下面将介绍，在 K8S 集群中通过 Bitnami 的 Helm Chart 搭建部署一个负载均衡架构 3 节点的 Postgres 集群

该集群将部署一个PGPool（Deployment）作为连接池，提供客户端连接数据库时的负载均衡作用
PG数据库节点（StatefulSet）作为数据库POD集，提供数据，同时相互之间通过`repmgr`（将存在于每一个节点POD中）进行复制

# 环境

- 腾讯云 kubernetes 1.18.4-tke.6
- Bitnami postgresql-ha-6.8.1 [Chart 下载](https://charts.bitnami.com/bitnami/postgresql-ha-6.8.1.tgz)
...
1. 手动创建 所需的云硬盘，这里我们有 3 个节点，需要 3 个云硬盘，每个 10GB
2. 创建与云硬盘对应的 `storageClass` 由于是数据库持久化型的数据，我们应该在生产环境中指定回收策略为 `Retain`，卷绑定模式应该设置为`立即绑定`
3. 创建指定`storageClass`的 PV 卷 并在控制台中与上一步创建的云硬盘操作进行绑定
4. PV 需要打上 labels
...
这里的 label 将在下面的步骤中用来对 PVC 进行匹配
...
基本参数与上面的PG节点差不多，但是由于pgpool是作为pg的负载均衡器存在（部署出来是一个Deployment），所以不需要挂载持久卷，仅需要按需求分配好pod副本数与资源即可
